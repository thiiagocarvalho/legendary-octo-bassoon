...
export async function GET() {
  ...
  const mineByOccurrence = new Map(mine.map((booking: (typeof mine)[number]) => [booking.occurrenceId, booking.id]));
  const makeupCredits = await prisma.makeupCredit.count({ where: { studentId: user.studentId, status: 'AVAILABLE' } });
  return NextResponse.json({ occurrences: occurrences.map((item: (typeof occurrences)[number]) => ({ id: item.id, startsAt: item.startsAt, endsAt: item.endsAt, capacity: item.classSlot.capacity, occupied: item.bookings.length, bookingId: mineByOccurrence.get(item.id) ?? null })), makeupCredits });
}
