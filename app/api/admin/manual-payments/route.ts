...
export async function GET() { try { await requireFinancialAccess(); return NextResponse.json(await prisma.manualPayment.findMany({ include: { enrollment: { include: { student: true } } }, orderBy: { receivedAt: 'desc' } })); } catch (error) { if (error instanceof UnauthorizedError) return NextResponse.json({ error: 'Acesso restrito ao administrador.' }, { status: 403 }); throw error; } }
export async function POST(request: Request) { try { const admin = await requireFinancialAccess(); return NextResponse.json(await recordManualPayment(await request.json(), admin.id), { status: 201 }); } catch (error) { if (error instanceof UnauthorizedError) return NextResponse.json({ error: 'Acesso restrito ao administrador.' }, { status: 403 }); const message = error instanceof Error ? error.message : 'Não foi possível registrar o recebimento.'; return NextResponse.json({ error: message }, { status: 400 }); } }
