...
import { deleteStudent } from '../../../../../../server/services/students';

export async function DELETE(_: Request, { params }: { params: Promise<{ studentId: string }> }) {
  try {
    const admin = await requireAdmin();
    const { studentId } = await params;
    return NextResponse.json(await deleteStudent(studentId, admin.id));
  } catch (error) {
    if (error instanceof UnauthorizedError) return NextResponse.json({ error: error.message }, { status: 403 });
    return NextResponse.json({ error: 'Não foi possível excluir o aluno.' }, { status: 400 });
  }
}
