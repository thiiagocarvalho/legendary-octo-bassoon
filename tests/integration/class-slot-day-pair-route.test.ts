...
const mocks = vi.hoisted(() => ({ requireOperationalAccess: vi.fn(), materializeOccurrences: vi.fn(), prisma: { classSlot: { findUnique: vi.fn(), update: vi.fn() } } }));
...
describe('PATCH /api/admin/class-slots/:classSlotId/day-pair', () => {
  ...
  it('rejects someone without operational access', async () => {
    mocks.requireOperationalAccess.mockRejectedValue(new UnauthorizedError('Sem permissão.'));
