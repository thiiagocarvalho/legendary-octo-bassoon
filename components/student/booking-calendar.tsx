...
type Occurrence = { id: string; startsAt: string; capacity: number; occupied: number; bookingId:string|null };
...
export function BookingCalendar() {
  ...
  async function cancel(bookingId:string){const response=await fetch(`/api/student/bookings/${bookingId}`,{method:'DELETE'});setMessage(response.ok?'Reserva cancelada; você pode escolher outro horário.':'A alteração só pode ser feita até 2 horas antes da aula.');if(response.ok)void load();}
  return <div className="grid gap-3">{items.map((item) => <article className="rounded-xl border bg-white p-4" key={item.id}><p className="font-semibold capitalize">{format(item.startsAt)}</p><p className="mt-1 text-sm text-slate-600">{item.capacity - item.occupied} vagas disponíveis</p>{item.bookingId?<button className="mt-3 rounded border border-amber-700 px-3 py-2 text-sm font-semibold text-amber-800" onClick={() => cancel(item.bookingId!)}>Cancelar / trocar horário</button>:<button className="mt-3 rounded bg-emerald-700 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={item.occupied >= item.capacity} onClick={() => reserve(item.id)}>Reservar</button>}</article>)}{message ? <p aria-live="polite">{message}</p> : null}{!items.length ? <p className="text-slate-600">Nenhum horário disponível.</p> : null}</div>;
}
