...
export function ProgressForm({ studentId }: { studentId: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const response = await fetch(`/api/admin/students/${studentId}/progress`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(Object.fromEntries(form)) }); setMessage(response.ok ? 'Evolução registrada.' : 'Não foi possível registrar.'); if (response.ok) { event.currentTarget.reset(); router.refresh(); } }
